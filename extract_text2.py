...
if success:
    ...
    print(width, height)
cv2.namedWindow("test2", flags=cv2.WINDOW_NORMAL)
count = 0
before_text = ""
align_API = Align_API()
i = 1
...
while success:
    ...
    if count == 5:
        count = 0
        text_image = image[text_box[0]-59:text_box[1]-55, text_box[2]:text_box[3]]
        cv2.imshow("test2", text_image)
    ...
    if result_name_list.__contains__("1") or result_name_list.__contains__("2"):
        ...
        if len(os.listdir(out_temp_path + str(i) + "/")) > 0:
            if len(os.listdir(out_temp_path+str(i)+"/")) > 4:
                for file_path in sorted(os.listdir(out_temp_path+str(i)+"/"))[2:len(os.listdir(out_temp_path+str(i)+"/"))-2]:
    ...
    save_img(out_temp_path+str(i)+"/", image[0:text_box[0]-59])
