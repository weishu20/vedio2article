...
if success:
    ...
    print(width, height)
cv2.namedWindow("test1", flags=cv2.WINDOW_NORMAL)
count = 0
before_text = ""
align_API = Align_API()
i = 1
...
while success:
    ...
    if count == 5:
        count = 0
        text_image = image[text_box[0]:text_box[1], text_box[2]:text_box[3]]
        cv2.imshow("test1", text_image)
    ...
    save_img(out_temp_path+str(i)+"/", image[0:text_box[0]-4])
